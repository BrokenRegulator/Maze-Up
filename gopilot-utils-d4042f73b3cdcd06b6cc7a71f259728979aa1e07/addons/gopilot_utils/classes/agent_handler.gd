...
func send_message(task:String) -> void:
	...
	response_since_observation = thought_delimiter
...
var response_since_observation:String = ""

## Internal function. Do not call directly
func _on_new_token(token:String) -> void:
	conversation[-1]["content"] += token
	response_since_observation += token
	
	var last_type:GenerationType = generation_type
	var has_observation:bool = observation_delimiter in response_since_observation
	var has_action:bool = action_delimiter in response_since_observation
	var has_task_finished:bool = final_answer_delimiter in response_since_observation
	var has_thought:bool = thought_delimiter in response_since_observation
	if action_delimiter in response_since_observation:
		generation_type = GenerationType.ACTION
	elif observation_delimiter in response_since_observation:
		generation_type = GenerationType.OBSERVATION
	elif final_answer_delimiter in response_since_observation:
		generation_type = GenerationType.THOUGHT
	if final_answer_delimiter in response_since_observation:
	...
	match generation_type:
		GenerationType.THOUGHT:
			new_thought_token.emit(token)
		GenerationType.ACTION:
			new_action_token.emit(token)
		GenerationType.FINAL_ANSWER:
			new_final_answer_token.emit(token)
			final_answer += token
	
	if has_thought and has_action:
		var thought:String = ""
		if debug_mode:
			print("case 1 string:\n", response_since_observation)
		thought = response_since_observation.split(action_delimiter)[0].split(thought_delimiter)[-1]
		response_since_observation = action_delimiter + response_since_observation.split(action_delimiter)[-1]
		thought_finished.emit(thought)
		return
	
	if has_thought and has_task_finished:
		var thought:String = ""
		thought = response_since_observation.split(final_answer_delimiter)[0].split(thought_delimiter)[-1]
		response_since_observation = final_answer_delimiter
		thought_finished.emit(thought)
		if debug_mode:
			print("case 2")
		return
	...
	if has_observation:
		var action:String = response_since_observation.split(observation_delimiter)[0].split(action_delimiter)[-1]
...
func _on_generation_finished(message:String):
	if generation_type == GenerationType.FINAL_ANSWER:
		task_finished.emit(response_since_observation.split(final_answer_delimiter)[-1])
		disconnect_signals()
		return
	if action_delimiter in response_since_observation and not observation_delimiter in response_since_observation:
		var action_string:String = response_since_observation.split(action_delimiter)[-1]
		# Adding observation delimiter so that _on_new_token handles the action
		response_since_observation += observation_delimiter
		_on_new_token("")
		return
